...
const TIMELINE = Array.from({ length: 14 }, (_, i) => {
  ...
  return { date: label, employee: Math.round(300 + Math.sin(i * 0.8) * 200 + Math.random() * 150), customer: Math.round(50 + Math.random() * 80) };
});
...
export default function DomainExposure() {
  ...
  return (
    <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 18, position: "relative" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18, animation: loaded ? "fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) both" : "none" }}>
        ...
        <div className="glass" style={{ padding: "20px", overflow: "hidden" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div className="mono" style={{ fontSize: 10, letterSpacing: "0.08em", color: t.text25, textTransform: "uppercase" }}>Credential Timeline</div>
            <span className="tab-btn on" style={{ padding: "3px 10px", fontSize: 10 }}>Employee</span>
          </div>
          <ResponsiveContainer width="100%" height={120}>
            <AreaChart data={TIMELINE}>
              ...
              <Tooltip {...ttS} />
              <Area type="monotone" dataKey="employee" stroke="#E8463A" fill="#E8463A" fillOpacity={0.08} strokeWidth={2} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
            ...
            <div style={{ flex: 1 }}>
              {RECORD_STATUS.map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  ...
                  <span className="mono" style={{ fontSize: 11, color: t.text60, fontWeight: 600 }}>{s.count > 1000 ? (s.count / 1000).toFixed(1) + "K" : s.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass" style={{ padding: "20px", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <MiniDonut segments={TOP_DOMAINS} />
            <div style={{ flex: 1 }}>
              {TOP_DOMAINS.map((d, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: d.color }} />
                  <span style={{ fontSize: 11, color: t.text50, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.label}</span>
                  <span className="mono" style={{ fontSize: 10, color: t.text35 }}>{d.count}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ═══ UNIQUE COUNTS ═══ */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18, animation: loaded ? "fadeUp 0.6s 0.1s cubic-bezier(0.16,1,0.3,1) both" : "none" }}>
        {[
          { label: "Unique FQDNs", value: "217", trend: "— 0.00%", down: true },
          { label: "Unique Passwords", value: "764", trend: "— 0.00%", down: true },
          { label: "Unique Usernames", value: "1.9K", trend: "— 0.00%", down: true },
        ].map((d, i) => (
          <div key={i} className="glass" style={{ padding: "18px 20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              <div>
                <span className="hfont" style={{ fontSize: 13, fontWeight: 700 }}>{d.label}</span>
                <span className="hfont" style={{ fontSize: 13, fontWeight: 700, marginLeft: 6 }}>{d.value}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <span className="mono" style={{ fontSize: 10, color: t.text30 }}>Last week</span>
                <span className="mono" style={{ fontSize: 10, color: "#16A34A" }}>{d.trend}</span>
              </div>
            </div>
            <svg width="100%" height="32" viewBox="0 0 200 32" preserveAspectRatio="none" style={{ display: "block", opacity: 0.4 }}>
              <polyline points={Array.from({ length: 20 }, (_, j) => `${j * 10.5},${16 + Math.sin(j * 0.6 + i * 1.5) * 10 - j * 0.2}`).join(" ")} fill="none" stroke="#A855F7" strokeWidth="1.5" strokeLinejoin="round" />
            </svg>
          </div>
        ))}
      </div>
      ...
      <div className="glass" style={{ overflow: "hidden", animation: loaded ? "fadeUp 0.6s 0.15s cubic-bezier(0.16,1,0.3,1) both" : "none" }}>
        ...
        {tab === "credentials" && (<>
          <div style={{ padding: "8px 20px", display: "grid", gridTemplateColumns: "28px 1fr 1fr 90px 90px 80px 70px 100px 80px 28px", gap: 8, borderBottom: `1px solid ${t.borderRow}`, alignItems: "center" }}>
          ...
          {sortData(filteredCreds).map((r, i) => (
            <div key={r.id} onClick={() => setSelectedCredId(r.id)} className="trow" style={{ display: "grid", gridTemplateColumns: "28px 1fr 1fr 90px 90px 80px 70px 100px 80px 28px", gap: 8, alignItems: "center", padding: "12px 20px", cursor: "pointer", background: activeSel.isSelected(r.id) ? "rgba(232,70,58,0.06)" : selectedCredId === r.id ? "rgba(232,70,58,0.04)" : i % 2 === 0 ? "rgba(255,255,255,0.008)" : "transparent", borderLeft: selectedCredId === r.id ? "3px solid #E8463A" : "3px solid transparent" }}>
              <Checkbox checked={activeSel.isSelected(r.id)} onChange={() => activeSel.toggle(r.id)} />
              <span className="mono" style={{ fontSize: 11, color: "#3B82F6", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.url}</span>
              <span style={{ fontSize: 11, color: t.text50, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.user}</span>
              <span className="mono" style={{ fontSize: 11, color: t.text40 }}>{r.password}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <div style={{ display: "flex", gap: 2 }}>
                  {[1, 2, 3, 4, 5].map(b => <div key={b} style={{ width: 10, height: 6, borderRadius: 1, background: b <= STR_VAL[r.strength] ? STR_COL[r.strength] : "rgba(255,255,255,0.06)" }} />)}
                </div>
              </div>
              <span style={{ padding: "3px 8px", borderRadius: 6, fontSize: 9, fontWeight: 600, background: "rgba(59,130,246,0.1)", color: "#3B82F6", fontFamily: "'JetBrains Mono',monospace" }}>{r.type}</span>
        ...
        {tab === "mentions" && (<>
          <div style={{ padding: "8px 20px", display: "grid", gridTemplateColumns: "28px 1fr 1fr 80px 110px 80px", gap: 8, borderBottom: `1px solid ${t.borderRow}`, alignItems: "center" }}>
          ...
          {filteredMentions.map(r => (
            <div key={r.id} onClick={() => setSelectedMentionId(r.id)} className="trow" style={{ display: "grid", gridTemplateColumns: "28px 1fr 1fr 80px 110px 80px", gap: 8, alignItems: "center", padding: "12px 20px", cursor: "pointer", background: activeSel.isSelected(r.id) ? "rgba(232,70,58,0.06)" : selectedMentionId === r.id ? "rgba(232,70,58,0.04)" : undefined, borderLeft: selectedMentionId === r.id ? "3px solid #E8463A" : "3px solid transparent" }}>
              <Checkbox checked={activeSel.isSelected(r.id)} onChange={() => activeSel.toggle(r.id)} />
              <span style={{ fontSize: 12, fontWeight: 500, color: t.text60 }}>{r.platform}</span>
              <span style={{ padding: "4px 10px", borderRadius: 6, fontSize: 11, background: "rgba(59,130,246,0.08)", color: "#3B82F6", fontFamily: "'JetBrains Mono',monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "inline-block", maxWidth: "100%" }}>{r.source}</span>
        ...
        <div style={{ padding: "12px 20px", borderTop: `1px solid ${t.borderRow}` }}>
